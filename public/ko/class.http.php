...
class Curl {
    ...
    public function upload_file($adres,$name,$file,$type="video/mp4"){
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, array("Content-type: multipart/form-data"));
        curl_setopt($this->curl, CURLOPT_URL, $adres);
        curl_setopt($this->curl, CURLOPT_POST, true);



        $postdata = array(
            $name => "@/".realpath($file).";type=$type" // virgül ile devam eder. array cünkü
        );
        //echo $postdata;
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $postdata);
        $sonuc = curl_exec ($this->curl);
        //echo "Sonuc ".$sonuc;
    }

    function download_image($img,$fullpath){
        //$ch = $this->curl;//curl_init ($img);
        curl_setopt($this->curl, CURLOPT_URL, $img);
    ...
    function Curl($timeout = 0) {
        $this->curl = curl_init();
        /*
        curl_setopt($this->curl, CURLOPT_PROXY, '209.164.72.72'); // brgzrdl..
        curl_setopt($this->curl, CURLOPT_PROXYPORT, '9786');
        curl_setopt($this->curl, CURLOPT_PROXYUSERPWD, "proxy786:p786");
        */


        $this->headers[] = "User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:28.0) Gecko/20100101 Firefox/28.0";
        $this->headers[] = "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8";
        $this->headers[] = "Accept-Language: tr-TR,tr;q=0.8,en-US;q=0.5,en;q=0.3";
        $this->headers[] = "Accept-Encoding: gzip, deflate";
        //$this->headers[] = "Referer: http://tr30.klanlar.org/game.php?village=10124&screen=overview";
        ...
        curl_setopt($this->curl, CURLOPT_USERAGENT, 'User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:28.0) Gecko/20100101 Firefox/28.0');
